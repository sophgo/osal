...
void *osal_calloc(unsigned long num, unsigned long size)
{
...
/* Define macro according to platform, for example, CONFIG_ARCH_X86 represents x86 architecture, if RISCV is not defined */
#ifdef CONFIG_ARCH_X86
#define MY_IOREMAP(phys, size) ioremap_cache((phys), (size))
#else
#define MY_IOREMAP(phys, size) ioremap((phys), (size))
#endif

void *osal_ioremap_wc(unsigned long phys_addr, unsigned long size)
{
    return MY_IOREMAP(phys_addr, size);
}
