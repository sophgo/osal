...
#define OSAL_TASK_PRIORITY_ABOVE_HIGH   10
#define OSAL_TASK_PRIORITY_HIGH         15
#define OSAL_TASK_PRIORITY_BELOW_HIGH   20
#define OSAL_TASK_PRIORITY_ABOVE_MIDDLE 25
#define OSAL_TASK_PRIORITY_MIDDLE       30
#define OSAL_TASK_PRIORITY_BELOW_MIDDLE 35
#define OSAL_TASK_PRIORITY_ABOVE_LOW    40
#define OSAL_TASK_PRIORITY_LOW          45
#define OSAL_TASK_PRIORITY_BELOW_LOW    60
#else
 ...
 *
 * @retval 1 if the process was woken up, 0 if it was already running.
 *
 * @par Support System:
 * linux alios rt_thread.
 */
 ...
 *
 * @attention
 * for up mode in RT-Thread, the underlying implementation of osal_kthread_bind is meaningless
 *
...
/**
 * @ingroup osal_task
 * @brief Exit the current kernel thread.
 *
 * @par Description:
 * This API is used to terminate the execution of the current kernel thread.
 *
 * @attention
 * This function should be called only when the thread is ready to terminate.
 * Improper use may lead to resource leaks or undefined behavior.
 *
 * @par Support System:
 * linux alios rt_thread.
 */
 ...
 * The current thread does not exit. The stop flag is not 0, this api will check whether the current thread exits
 *
 * @par Support System:
 * linux alios rt_thread.
 */
 ...
 *
 * @attention
 * this api is availale when defines RT_USING_SMART
 *
...
/**
 * @ingroup osal_task
 * @brief sleep for an approximate time.
 *
 * @par Description:
 * This API is used to sleep for a period of usecs.
 *
 * @param min [in] Minimum time in usecs to sleep.
 * @param max [in] Maximum time in usecs to sleep.
 *
 * @par Support System:
 * linux alios rt_thread.
 */
